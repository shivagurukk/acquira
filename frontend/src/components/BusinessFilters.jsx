...
// Custom dark styled TextField for consistency
...
// Custom Autocomplete for specific styling
const DarkAutocomplete = ({ label, options, value, onChange, placeholder }) => (
    <Autocomplete
        multiple
        options={options || []}
        value={value || []}
            ...
            value.map((option, index) => (
                <Chip
                    ...
                    sx={{
                        ...
                        color: 'white',
                        fontWeight: 600,
                        '& .MuiChip-deleteIcon': { color: 'white !important', opacity: 0.7 }
                    }}
                />
            ))
        }
        ...
        PaperComponent={({ children }) => (
            <Box sx={{ bgcolor: '#1F2937', color: 'white', '& .MuiMenuItem-root': { '&:hover': { bgcolor: '#374151' } } }}>
                {children}
            </Box>
        )}
    />
...
const BusinessFilters = ({ filters, onChange, onApply, isOpen, onClose }) => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    setOptions(prev => ({
                        ...
                        // Ensure mccs is array of strings
    ...
    return (
        <Drawer
            ...
            onClose={onClose}
            PaperProps={{
                sx: { width: 450, bgcolor: '#111827', color: '#F8FAFC', borderLeft: '1px solid #1F2937' }
            }}
        >
            {/* Header */}
            <Box sx={{ p: 3, borderBottom: '1px solid #1F2937', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Stack direction="row" spacing={1.5} alignItems="center">
                    ...
                    <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">Apply filters to narrow down results</Typography>
                    </Box>
                </Stack>
            ...
            <Box sx={{ p: 3, flex: 1, overflowY: 'auto' }}>
                ...
                {/* 1. Merchant Search */}
                <Box mb={4}>
                    <Typography variant="overline" color="text.secondary" fontWeight="700" display="block" mb={1} letterSpacing={1}>
                        Merchant Search
                    </Typography>
                    <DarkTextField
                ...
                <Box mb={4}>
                    ...
                    <ToggleButtonGroup
                        value={dateType}
                        exclusive
                        onChange={(e, val) => val && setDateType(val)}
                        fullWidth
                        size="small"
                ...
                <Box mb={4}>
                    ...
                    <Stack spacing={2}>
                        <DarkAutocomplete
                            ...
                            value={filters.partnerList}
                            onChange={(v) => update('partnerList', v)}
                            placeholder="All Partners"
                        />
                        <DarkAutocomplete
                            label="Relationship Manager"
                            options={options.rms}
                            value={filters.rmList}
                            onChange={(v) => update('rmList', v)}
                            placeholder="All RMs"
                        />
                        <DarkAutocomplete
                            label="Team Leader"
                            options={options.teamLeaders}
                            value={filters.teamLeaderList}
                            onChange={(v) => update('teamLeaderList', v)}
                ...
                <Box mb={4}>
                    ...
                    <Stack spacing={2}>
                        <DarkAutocomplete
                            ...
                            onChange={(v) => update('mccList', v)}
                            placeholder="All MCCs"
                        />
                        <DarkAutocomplete
                            label="Industry"
                            options={options.industries}
                            value={filters.industryList}
                            onChange={(v) => update('industryList', v)}
                            placeholder="Select Industries"
                        />
                        <DarkAutocomplete
                            label="Sector"
                            options={options.sectors}
                            value={filters.sectorList}
                            onChange={(v) => update('sectorList', v)}
                ...
                <Box mb={2}>
                    ...
                    <Box display="grid" gridTemplateColumns="1fr 1fr" gap={2}>
                        <DarkAutocomplete
                            ...
                            value={filters.channelList}
                            onChange={(v) => update('channelList', v)}
                            placeholder="All"
                        />
                        <DarkAutocomplete
                            label="Destination"
                            ...
                            onChange={(v) => update('destinationList', v)}
                            placeholder="All"
                        />
                        <DarkAutocomplete
                            label="Scheme"
                            ...
                            onChange={(v) => update('schemeList', v)}
                            placeholder="All"
                        />
                        <DarkAutocomplete
                            label="Card Type"
                            ...
                            onChange={(v) => update('cardTypeList', v)}
                            placeholder="All"
                        />
                    </Box>
                </Box>
