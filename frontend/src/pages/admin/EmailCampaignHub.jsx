...
const TEMPLATE_TYPES = [
  ...
  { value: 'REPORT_PDF', label: 'PDF report email' },
...
// The PDF report email is generated by the batch, not by a campaign, so it is
// handed a different — and much smaller — set of values. Offering the campaign
// list here would let someone insert {{total_volume}} into an email that never
// receives it, and it would render as empty. Mirrors
// ReportEmailTemplateService.REPORT_VARIABLES on the backend.
const REPORT_PDF_MERGE_VARS = ['merchant_name', 'mid', 'month_year', 'statement_month', 'pdf_filename'];

const varsForType = (type) => (type === 'REPORT_PDF' ? REPORT_PDF_MERGE_VARS : MERGE_VARS);
...
const TemplatesTab = () => {
  ...
  // Load the built-in PDF report email as the starting point. That email is what
  // the platform sends today, so editing a copy of it is the realistic way to
  // customise it — writing an Outlook-safe table layout from an empty box is not.
  const loadBuiltIn = async () => {
    setLoadingBuiltIn(true);
    try {
      const r = await api.get(`/email-campaigns/templates/builtin/${form.templateType}`);
      if (!r.data?.hasBuiltIn) {
        showToast('No built-in template for this type', 'info');
        return;
      }
      setForm(f => ({
        ...f,
        subjectTemplate: r.data.subjectTemplate,
        bodyHtml: r.data.bodyHtml,
      }));
      showToast('Built-in template loaded — edit and save to override it', 'success');
    } catch {
      showToast('Could not load the built-in template', 'error');
    ...
    <Stack gap="sm">
      ...
      >
        <div className="ui-stack ui-stack--sm">
          ...
          {form.templateType === 'REPORT_PDF' && (
            <Alert tone="info" title="PDF report email">
              This is the covering email merchants receive with their Business Insight
              Report attached. Mark it <strong>default for type</strong> and the monthly
              PDF batch will use it instead of the built-in email. Without one, the
              built-in email is sent unchanged.
          ...
          <FormField label="Subject line" hint="Merge variables are substituted per recipient.">
            <Input
              ...
              placeholder={form.templateType === 'REPORT_PDF'
                ? 'Your Business Insight Report — {{month_year}}'
                : 'Your {{month}} statement - {{merchant_name}}'}
            />
          </FormField>

          <FormField
            label="Merge variables"
            hint={form.templateType === 'REPORT_PDF'
              ? 'The values the PDF batch supplies. Select one to append it to the email body.'
              : 'Select one to append it to the email body.'}
          >
            <div className="ui-row" style={{ gap: 4 }}>
              {varsForType(form.templateType).map(v => (
                <Button key={v} type="button" size="sm" variant="subtle" onClick={() => insertVar(v)}>
                  {`{{${v}}}`}
                </Button>
              ))}
            </div>
          ...
          {/* isActive / isDefaultForType were already persisted by the API but had
              no control here, so a template could never actually be made the
              default — the one flag the PDF batch looks up. */}
          <FormGrid cols={2}>
            <Checkbox
              label="Active"
              hint="Inactive templates are ignored at send time."
            ...
            <Checkbox
              ...
              hint={form.templateType === 'REPORT_PDF'
                ? 'The template the monthly PDF batch sends.'
                : 'Used when a send picks this type without naming a template.'}
              checked={!!form.isDefaultForType}
              onChange={e => setForm({ ...form, isDefaultForType: e.target.checked })}
            />
          </FormGrid>
        </div>
      </Modal>
