...
const FinanceSummary = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:8080/api/finance/summary?${query}`, {
    ...
    const fetchDailyData = async (monthLabel, avgDate) => {
        ...
        try {
            ...
            const token = localStorage.getItem('token');
            const res = await fetch(`http://localhost:8080/api/finance/summary?period=CUSTOM&groupBy=DAY&startDate=${startStr}&endDate=${endStr}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
    ...
    const fetchMerchantData = async (dateStr) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`http://localhost:8080/api/finance/summary?period=CUSTOM&groupBy=MERCHANT&startDate=${dateStr}&endDate=${dateStr}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
                const list = await res.json();
