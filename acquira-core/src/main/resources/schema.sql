...
CREATE TABLE IF NOT EXISTS ref_country (
    ...
    iso_numeric VARCHAR(3),              -- CAUTION: holds ISO 4217 CURRENCY numerics in practice (BR=986/BRL)
    iso3166_numeric VARCHAR(3),          -- real ISO 3166-1 COUNTRY numeric (BR=076); seeded below, used by BIN promotion
...
-- Real ISO 3166-1 numeric country codes (V2026_08_14_02, kept here because
-- this file DROPs+recreates ref_country every dev boot — a migration-only fix
-- is wiped on restart). iso_numeric holds CURRENCY numerics and must NOT be
-- used for country resolution; Mastercard BIN promotion reads iso3166_numeric.
