...
WITH mea(cc) AS (VALUES  -- Mastercard MEA region issuer countries (edit as needed)
...
intl AS MATERIALIZED (
  ...
  WHERE UPPER(TRIM(COALESCE(ft.destination,''))) = 'INTERNATIONAL'
...
priced AS (
  SELECT i.tenant_id, i.channel, i.amt, i.current_fee,
...
FROM priced
GROUP BY tenant_id, ROLLUP ((intra_mea, card_tier, card_class, channel))
